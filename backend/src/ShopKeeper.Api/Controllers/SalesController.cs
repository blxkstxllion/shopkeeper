...
public class SalesController(ISender mediator) : ControllerBase
{
    ...
    {
        await mediator.Send(new VoidSaleCommand(id, body.Reason), ct);
        return NoContent();
    }

    [HttpPost("{id:guid}/refund")]
    public async Task<ActionResult<RefundDto>> Refund(Guid id, [FromBody] RefundSaleRequest body, CancellationToken ct) =>
        Ok(await mediator.Send(new RefundSaleCommand(id, body.Items, body.Reason), ct));
...
public record RefundSaleRequest(IReadOnlyList<RefundLineInput> Items, string Reason);
