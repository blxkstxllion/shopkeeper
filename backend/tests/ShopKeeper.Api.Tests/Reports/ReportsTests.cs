...
public class ReportsTests : IDisposable
{
    ...
    public async Task ProfitabilityReport_ExcludesSalesAndExpensesOutsideRange()
    {
        ...
        var juneSale = await new CreateSaleCommandHandler(context, owner).Handle(
        ...
        var augustSale = await new CreateSaleCommandHandler(context, owner).Handle(
    ...
    public async Task ProfitabilityReport_TopAndWorstProducts_ReflectRealProfit()
    {
        ...
        var goodProduct = await new CreateProductCommandHandler(context, owner).Handle(
            new CreateProductCommand("High Margin", "SKU-HI", null, null, null, null, 20m, 5m, 5, 10, true, 10, seeded.BranchId),
            CancellationToken.None);
        var badProduct = await new CreateProductCommandHandler(context, owner).Handle(
