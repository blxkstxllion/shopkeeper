...
6. On the host itself, create `/opt/shopkeeper/.env` with the production values from `.env.example` — `POSTGRES_PASSWORD`, `JWT_SECRET`, `IMAGE_OWNER`, `DOMAIN`, `ACME_EMAIL`, `EMAIL_FROM_ADDRESS`, `BACKUP_S3_BUCKET`, `PAYSTACK_SECRET_KEY` (and the 4 `PAYSTACK_*_PLAN_CODE` values), etc. This file is never touched by CI/CD, only by whoever administers the host.
