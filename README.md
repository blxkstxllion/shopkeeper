...
1. Provision a host with Docker installed, and this repo (or at least `docker/` and `database/`) checked out at `/opt/shopkeeper`. A single small EC2 instance (or equivalent VPS) is sufficient — see the architecture assessment for why this app doesn't need ECS/Fargate/Kubernetes. If this host also serves another app (e.g. the `omnifixlabs` portfolio site, checked out separately at `/opt/omnifixlabs`), `docker network create public-web` once — both apps' frontends and the shared Caddy instance below join it.
2. **Grant AWS access scoped to only**:
   ...
   Nothing in this app needs broader AWS permissions than that. **On EC2**, attach an IAM instance role with this policy instead of issuing a static access key — the AWS SDK finds it automatically, so `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` stay unset in `.env`. **On any other host** (Hetzner, DigitalOcean, etc. — no instance metadata service to fall back to), create an IAM user with the same scoped policy and put its access key in `.env` instead — see `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` in `.env.example`.
...
6. On the host itself, create `/opt/shopkeeper/.env` with the production values from `.env.example` — `POSTGRES_PASSWORD`, `JWT_SECRET`, `IMAGE_OWNER`, `DOMAIN`, `ROOT_DOMAIN`, `ACME_EMAIL`, `EMAIL_FROM_ADDRESS`, `BACKUP_S3_BUCKET`, `PAYSTACK_SECRET_KEY` (and the 4 `PAYSTACK_*_PLAN_CODE` values), etc. This file is never touched by CI/CD, only by whoever administers the host.
7. Set up the nightly backup cron job — see `database/backup-restore.md`.
8. **If enabling real billing**: create a Paystack account, grab a secret key from Settings → API Keys & Webhooks, and create the 4 paid-tier Plans by hand in the dashboard (their PLN_xxx codes go in `.env` — see `.env.example`). Then register `https://<DOMAIN>/api/webhooks/paystack` as the webhook URL for that mode (test or live) in the same dashboard page. Until `PAYSTACK_SECRET_KEY` is set, plan changes stay exactly as self-serve as they are today.
9. Start the shared Caddy instance once, from either app's checkout since it's its own compose project: `docker compose -f docker/docker-compose.caddy.yml --env-file .env up -d`. It's the sole public entry point (ports 80/443) — this app's own `frontend` container is never published to the host directly (see `docker-compose.prod.yml`).
...
- **TLS**: a single shared `caddy` instance (its own compose project, `docker/docker-compose.caddy.yml` — not part of this app's own stack) is the sole public entry point in production (ports 80/443), automatically obtaining and renewing Let's Encrypt certificates and terminating TLS in front of this app's Nginx frontend — no manual certbot/renewal process to maintain. If the same host also serves another app, that app's frontend joins the same external `public-web` network and gets its own site block in `docker/Caddyfile`. See that file for the routing.
...
- **Billing**: real subscription payments go through Paystack's Subscriptions API (`PaystackClient`) once `Paystack:SecretKey` is configured — checkout is a redirect to Paystack's hosted page, activation happens synchronously when the user's browser returns (`VerifyCheckoutCommand`), and a signed webhook (`/api/webhooks/paystack`, verified via HMAC-SHA512, no JWT) keeps subscription status in sync afterward. Local/CI dev has no `Paystack:SecretKey` configured and keeps today's fully self-serve plan changes.
